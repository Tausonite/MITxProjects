...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        displayHand(hand)
        # Ask user for input
        freshWord = input("Enter word guess: ")
        ...
        # Otherwise (the input is not a single period):
            ...
            if not isValidWord(freshWord, hand, wordList):
                # Reject invalid word (print a message followed by a blank line)
                print("Word invalid.")
                print()
            # Otherwise (the word is valid):
            else:
                totalScore += getWordScore(freshWord, n)
                # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                print("Word is worth " + str(getWordScore(freshWord, n)) + ", your score is now " + str(totalScore) + ".")
                print()
                # Update the hand 
                hand = updateHand(hand, freshWord)

    # Game is over (user entered a '.' or ran out of letters), so tell user the total score
    return "You're out of letters. Your total score was: " + str(totalScore) + " points."
